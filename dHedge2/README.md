...
## Building and Running the dHedge2 Application

In order to build the dHedge2 application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the dHedge2 application's code.
```
fprime-util build
```
...
To run the ground system without starting the dHedge2 app:
```
...
```
cd dHedge2/build-artifacts/<platform>/bin/
./dHedge2 -a 127.0.0.1 -p 50000
```
